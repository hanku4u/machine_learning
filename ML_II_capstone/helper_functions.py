...
def add_noise_to_samples(df, n_samples, noise_level=0.05, sensor_limits=None, sensors_to_modify=None):
    """
...
def add_cd_values(df: pd.DataFrame, sensor_limits: dict, cd_targets: dict) -> pd.DataFrame:
    ...
    # sensors excluding 'RF' and 'InSpec' if present
    sensors = [sensor for sensor in df.columns if sensor not in ['RF', 'InSpec']]

    # calculate relative deviations for all sensors
    ...
    # determine if sensors are significantly beyond limits
    ...
    for sensor in sensors:
        ...
        # avoid division by zero for fixed sensors
        if sensor_range == 0:
    ...
    # for each CD calculate value based on sensor deviations
    for cd in cd_targets.keys():
        upper_cd, lower_cd, target_cd = cd_targets[cd]
        df_with_cd[cd] = target_cd # initialize CD value at target

        # For simplicity, assign equal base weights
        base_weight = 1.0
        ...
        # add extra weight to sensors significantly high or low
        extra_weight_factor = 5.0  # Adjust this factor as needed
        for sensor in sensors:
            weights.loc[significant_deviation[sensor], sensor] *= extra_weight_factor

        # calculate weighted sum of deviations
        weighted_deviation = (weights * deviations).sum(axis=1)

        # scale the adjustment to CD's range
        cd_range = upper_cd - lower_cd
        df_with_cd[cd] += weighted_deviation * (cd_range / 2)
